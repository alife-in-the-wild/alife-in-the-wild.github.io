<!--
  CALL FOR PAPERS — edit this file to change CFP content.

  FORMAT:
    - Cards are separated by a horizontal rule: `---`
    - The FIRST chunk (before the first `---`) is the optional intro paragraph(s).
    - Each subsequent chunk is ONE card and follows this shape:

        ## Card title
        Tagline shown in accent type (length / format / template — one short line).
