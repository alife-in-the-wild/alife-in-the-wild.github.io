...
/**
 * SPEAKERS
 *   Each `---`-separated chunk is one speaker:
 *     `## Name` → name, paragraph 1 → affiliation, paragraph 2 → talk,
 *     remaining paragraphs → blurb.
 *   A blockquote anywhere becomes the shared footnote.
 */
export function parseSpeakers(md) {
  const speakers = [];
  let footnoteHtml = '';

  for (const chunk of splitOnHr(md)) {
    const tokens = lex(chunk);

    const quote = tokens.find(t => t.type === 'blockquote');
    if (quote) {
      footnoteHtml = quote.tokens
        .filter(t => t.type === 'paragraph')
        .map(p => renderParagraphHtml(p))
        .join(' ');
    }
    ...
    const paragraphs = tokens.filter(t => t.type === 'paragraph');
    speakers.push({
      name: renderInline(heading.tokens),
      affiliation: paragraphs[0] ? renderParagraphHtml(paragraphs[0]) : '',
      talkHtml: paragraphs[1] ? renderParagraphHtml(paragraphs[1]) : '',
      bodyHtml: paragraphs
        .slice(2)
        .map(p => `<p>${renderParagraphHtml(p)}</p>`)
        .join(''),
    });
  }

  return { speakers, footnoteHtml };
}

/**
 * SCHEDULE (running order)
 *   Each `---`-separated chunk is one group: `## Label` plus a GFM table of
 *   the same shape `parseDates` reads. The column header comes from the first
 *   table; a blockquote anywhere becomes the footnote.
 */
export function parseSchedule(md) {
  const groups = [];
  let header = [];
  let footnoteHtml = '';
  ...
  for (const chunk of splitOnHr(md)) {
    ...
    const table = tokens.find(t => t.type === 'table');
    if (!table) continue;
    if (header.length === 0) header = table.header.map(h => renderInline(h.tokens));
  ...
  return { header, groups, footnoteHtml };
}

/**
 * ACCEPTED PAPERS
 *   Each `---`-separated chunk is one accepted contribution:
 *     `## Title` → title, paragraph 1 → authors, paragraph 2 → affiliation,
 *     paragraph 3 → track.
 *   A blockquote anywhere becomes the shared footnote.
 */
export function parseAccepted(md) {
  const papers = [];
  let footnoteHtml = '';
  ...
  for (const chunk of splitOnHr(md)) {
    ...
    const paragraphs = tokens.filter(t => t.type === 'paragraph');
    papers.push({
      title: renderInline(heading.tokens),
      authorsHtml: paragraphs[0] ? renderParagraphHtml(paragraphs[0]) : '',
      affiliationHtml: paragraphs[1] ? renderParagraphHtml(paragraphs[1]) : '',
      trackHtml: paragraphs[2] ? renderParagraphHtml(paragraphs[2]) : '',
    });
  }

  return { papers, footnoteHtml };
}
