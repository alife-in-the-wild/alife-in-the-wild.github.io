...
/**
 * SPEAKERS
 *   Each chunk → { name, affiliationHtml, bioHtml }
 *   First paragraph after the heading = affiliation; rest = bio.
 */
export function parseSpeakers(md) {
  return splitOnHr(md).map(chunk => {
    ...
    const heading = tokens.find(t => t.type === 'heading' && t.depth === 2);
    const paragraphs = tokens.filter(t => t.type === 'paragraph');
    const affil = paragraphs[0];
    const bio   = paragraphs.slice(1);
    return {
      name: heading ? renderInline(heading.tokens) : '',
      affiliationHtml: affil ? renderParagraphHtml(affil) : '',
      bioHtml: bio.map(p => `<p>${renderParagraphHtml(p)}</p>`).join(''),
    };
  });
}
